...
* **Direct web connectivity** — Connect from any web browser on macOS, Windows, Linux, or mobile devices directly to AROMA over your local WiFi network (no app installation needed on the client).
* **Remote access** from any browser using a public URL (via ngrok tunnel).
...
* 5b.If uploads fail, open `http://<phone-ip>:8080/_aroma_diag` in your browser to view server-side diagnostic logs with timestamps.
...
## Nearby File Transfer (LocalSend)

AROMA supports peer-to-peer file transfer via the [LocalSend protocol](https://localsend.org) (Apache 2.0).

### LocalSend
- Uses **UDP multicast** for discovery — requires both devices on the **same WiFi router/network**
- Uses **HTTPS** over that same network for file transfer
- Cannot work without a shared WiFi network; also works over a **WiFi hotspot** (one device creates a hotspot, the other connects to it)
- Install **[LocalSend](https://localsend.org)** (free, open source) on your Mac/iPhone/Windows/Linux to transfer files to/from AROMA

### Apple AirDrop — not supported
- Uses **Bluetooth** for discovery
- Uses **WiFi Direct** (peer-to-peer, no router needed) for transfer
- Works with zero network infrastructure
- AirDrop is Apple's closed proprietary protocol and is not compatible with Android apps
